<template>
  <div class="table-box">
    <ProTable ref="proTable" title="交易记录" :columns="columns" :requestApi="VersionApi.page" :initParam="initParam" :dataCallback="dataCallback">
      <!-- 表格 header 按钮 -->
...
// 删除数据
const deleteRowData = async (params: Cardbag) => {
...
// 打开 drawer(新增、查看、编辑)
const dialogRef = ref()
const openDrawer = (title: string, row: Partial<Cardbag> = {}) => {
  let params = {
    title,
    row: { ...row },
    isView: title === '查看',
